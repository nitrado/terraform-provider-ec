...
func TestResourceArmadaSets(t *testing.T) {
	name := "my-armadaset"
	env := "dflt"
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy:      testCheckArmadaSetsDestroy(cs),
		Steps: []resource.TestStep{
			{
				Config: testResourceArmadaSetsConfigBasic(env, name),
			...
			{
				Config: testResourceArmadaSetsConfigBasicWithEnv(env, name),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("ec_armada_armadaset.test", "metadata.0.name", name),
					resource.TestCheckResourceAttr("ec_armada_armadaset.test", "metadata.0.environment", env),
					resource.TestCheckResourceAttr("ec_armada_armadaset.test", "spec.#", "1"),
...
func testResourceArmadaSetsConfigBasic(env, name string) string {
...
func testResourceArmadaSetsConfigBasicWithEnv(env, name string) string {
	return fmt.Sprintf(`resource "ec_armada_armadaset" "test" {
  metadata {
    name = "%s"
    environment = "%s"
...
func testCheckArmadaSetsDestroy(cs clientset.Interface) func(s *terraform.State) error {
